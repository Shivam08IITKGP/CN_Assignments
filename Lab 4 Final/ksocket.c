...
int dropMessage(float p)
{
    ...
    double seed = tv.tv_sec + (tv.tv_usec / 1e6);

    // Generate a probability (0 to 1)
    double probability = (seed - (int)seed);
...
int k_sendto(int sockfd, const void *message, size_t length)
{
    ...
    printf("Seq Num: %d is stored in %d\n", seq_num, free_index);

    printf("Sockfd = %d\n", sockfd);
    printf("Seq Num = %d\n", seq_num);
    printf("Free Index = %d\n", free_index);
    printf("shared_memory[sockfd].udp_socket = %d\n", shared_memory[sockfd].udp_socket);
    printf("shared_memory[i].is_free = %d\n", shared_memory[sockfd].is_free);
    shared_memory[sockfd].swnd.index_seq_num[seq_num] = free_index;
    printf("shared_memory[sockfd].swnd.index_seq_num[seq_num] = %d\n", shared_memory[sockfd].swnd.index_seq_num[seq_num]);
...
int k_recvfrom(int sockfd, void *buffer, size_t length)
{
    ...
    if (sem_SM == SEM_FAILED)
    {
        perror("k_recvfrom: Failed to open sem_SM");
        return -1;
    }
